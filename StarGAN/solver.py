...
class Solver(object):
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            out_src, out_cls = self.D(x_real) # 分辨是真假照片 | 分類照片的 label
            d_loss_real = - torch.mean(out_src) # 判斷真假照片的 loss，因為都是真照片輸入，因此判斷成假照片時 loss 要比較大
            d_loss_cls = self.classification_loss(out_cls, label_org, self.dataset) # 算真 label 與 D 分出來的 label 差別

            # Compute loss with fake images.
            x_fake = self.G(x_real, c_trg) # 輸入真照片並指定轉換的 label 給 G，輸出假照片
            out_src, out_cls = self.D(x_fake.detach()) # D 要盡可能判斷出是 G 產生的
            d_loss_fake = torch.mean(out_src) # 如果依然判斷成真照片，loss 要比較大
            ...
            if (i+1) % self.n_critic == 0:
                ...
                x_fake = self.G(x_real, c_trg) # 輸入真照片並指定轉換的 label 給 G，輸出假照片
                out_src, out_cls = self.D(x_fake) # D 要盡可能判斷出是 G 產生的
                g_loss_fake = - torch.mean(out_src) # 如果 D 判斷 G 產生的照片是真的，Loss 會比較小
                g_loss_cls = self.classification_loss(out_cls, label_trg, self.dataset) # D 分出來的 label 與 指定轉換的 label 的誤差

                # Target-to-original domain.
                x_reconst = self.G(x_fake, c_org) # 將真照片原本的 label + 假照片當作輸入，要復原的原本的真照片
                g_loss_rec = torch.mean(torch.abs(x_real - x_reconst)) # 原來的照片與復原的照片的誤差
